...
export default function ContainerForm({
  ...
  const { payees, accounts } = useSelector((state) => state.accountState);
            ...
            >
              {!isOTP && (
                <>
                  ...
                  >
                    ...
                    {(isUpSm || !['TransferTo', 'Amount'].includes(step)) && (
                      <>
                        <Typography
                          variant="subtitle2"
                          fontFamily="Roboto"
                          color={(theme) =>
                            theme.palette.grey[!['TransferTo'].includes(step) ? 900 : 300]
                          }
                          mb={!['TransferTo'].includes(step) ? 1 : 4}
                        >
                          Amount
                        </Typography>

                        {!['TransferTo'].includes(step) && (
                          <>
                            {!amount ? (
                              <Typography
                                variant="subtitle2"
                    ...
                    {(isUpSm || !['TransferTo', 'Amount', 'SelectAnAccount'].includes(step)) && (
                      <>
                        <Typography
                          variant="subtitle2"
                          fontFamily="Roboto"
                          color={(theme) =>
                            theme.palette.grey[!['TransferTo', 'Amount'].includes(step) ? 900 : 300]
                          }
                          mb={!['TransferTo', 'Amount'].includes(step) ? 1 : 4}
                        >
                          Transfer from
                        </Typography>

                        {!['TransferTo', 'Amount'].includes(step) && (
                          <>
                            {!card ? (
                              <Typography
                                variant="subtitle2"
                    ...
                    {!isLast &&
                      (isUpSm ||
                        !['TransferTo', 'Amount', 'SelectAnAccount', 'ConfirmAction'].includes(
                          step
                        )) && (
                        <>
                          <Typography
                            ...
                            color={(theme) =>
                              theme.palette.grey[
                                !['TransferTo', 'Amount', 'SelectAnAccount'].includes(step)
                                  ? 900
                                  : 300
                              ]
                            }
                            mb={!['TransferTo', 'Amount', 'SelectAnAccount'].includes(step) ? 1 : 4}
                          >
                            Confirm transfer
                          </Typography>
                    ...
                    {!isUpSm && !isLast && !['TransferTo'].includes(step) && <Divider />}
                  </Stack>
                </>
              )}
              ...
              {!isLast && (
                <>
                  ...
                  <Stack
                    pt={{ sm: !['TransferTo'].includes(step) ? 7 : 0 }}
                    maxWidth={{ sm: 300, md: 320 }}
                    width="100%"
                  >
                    {children}
